...
import { saveSelectedTabs, saveCurrentWindow } from "@/utils/tabs";
  ...
  return (
    <div class="w-[300px] h-[300px] flex flex-col items-center justify-between p-1 bg-gray-100 rounded-sm">
      <div class="flex flex-col grow text-center items-center p-4">
        <Show
          when={tabCount.state === "ready" || tabCount.state === "refreshing"}
          fallback={<Skeleton height={72} width={62} radius={10} />}
        >
          <p class="text-7xl font-extrabold text-gray-800">{tabCount()}</p>
        </Show>
        <p>Tabs Saved</p>
      </div>
        ...
        <For
          each={
            [
              ...
              ["Open Options", () => switchToOrOpenTab(URLS.OPTIONS)],
              ["Open History", () => switchToOrOpenTab(URLS.HISTORY)],
              ["Seed", addSeedTabs],
              [
                "Clear All",
                async () => {
